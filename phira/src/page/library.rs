...
pub struct LibraryPage {
    ...
    selected_chart_index: Option<usize>,
...
impl LibraryPage {
    ...
    fn sync_local(&mut self, s: &SharedState) {
        ...
        if list.ty == ChartListType::Local {
            ...
            for (folder_path, _) in &s.folders {
                // 检查这个文件夹是否应该显示在当前目录下
                if let Some(ref cur_path) = current_path {
                    // 在子文件夹中
...
impl Page for LibraryPage {
    ...
    fn update(&mut self, s: &mut SharedState) -> Result<()> {
        ...
        if let Some(_chart_index) = self.tabs.selected_mut().view.clicked_menu.take() {
            ...
            sorted_folders.sort();
            folder_options.extend(sorted_folders.clone());
            ...
            if id == "search" {
                ...
                if !text.is_empty() {
                    ...
                    let new_folder_path = if self.current_folder.is_empty() {
                        ...
                        format!("{}/{}", self.current_folder.join("/"), text)
                    };
                    // 将文件夹添加到 SharedState
                    let data = get_data_mut();
                    data.folders.insert(new_folder_path.clone(), false);
                    save_data().ok();
                    show_message(tl!("folder-created", "name" => text)).ok();
                    // 重新加载本地铺面数据
                    s.reload_local_charts();
                    // 刷新显示
                    self.sync_local(s);
                ...
                if !text.is_empty() && !self.current_folder.is_empty() {
                    ...
                    save_data().ok();
                    show_message(tl!("folder-renamed", "name" => text.clone())).ok();
        ...
        if self.tabs.selected_mut().view.need_update() {
            ...
            if !self.current_folder.is_empty() && self.charts_count_before_import > 0 {
                ...
                if current_count > self.charts_count_before_import {
                    ...
                    if moved_count > 0 {
                        save_data().ok();
                        show_message(tl!("imported-to-folder", "folder" => target_folder)).ok();
                    }
                }
            }
